...
import { outfarehelper } from '../common/OutFareCalculator';
...
export default class OutfareScreen extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
        ...
        Data.once('value', rates => {
            if (rates.val()) {
                ...
                for (var i = 0; i < carTypeWiseRate.car_type.length; i++) {
                    if (carTypeWiseRate.car_type[i].name == carType && carTypeWiseRate.car_type[i].category == 'Keke') {
    ...
    async getDirections(startLoc, destLoc) {
        try {
            ...
            var fareCalculation = outfarehelper(respJson.routes[0].legs[0].distance.value, respJson.routes[0].legs[0].duration.value, this.state.rateDetails ? this.state.rateDetails : 1)
            this.setState({
                distance: respJson.routes[0].legs[0].distance.value,
                rpkm: fareCalculation ? parseFloat(fareCalculation.ratePerKm).toFixed(2) : 0,
                    ...
                    <View style={styles.modalInnerContainer}>
                        ...
                        <View style={styles.modalInner}>
                            ...
                            <View style={styles.fareDetailsContainer}>
                                    ...
                                    <Text style={styles.fareTitleText}>Est Rate/KM</Text>
                                    <Text style={styles.fareTitleText}>Est KM</Text>
                                        <Text style={styles.fareTitleText}>Estimated Cost</Text>
                                        <Text style={styles.fareTitleText} style={{display:'none'}}>{languageJSON.convenience_free}</Text>
                                    </View>
                                    <View style={styles.verticalLine} />
                                    <View style={styles.farePriceContainer}>
                                        <Text style={styles.farePriceText}>{Currency} {this.state.rpkm} </Text>
                                        <Text style={styles.farePriceText}>{this.state.distance/1000} KM </Text>
                                        <Text style={styles.farePriceText}>{Currency} {this.state.fareCost} </Text>
                                        <Text style={styles.farePriceText} style={{display:'none'}}>{Currency} {this.state.convenience_fees ? this.state.convenience_fees : 0} </Text>
                                    </View>
                                </View>
