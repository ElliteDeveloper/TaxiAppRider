...
import { rentalfarehelper } from "../common/RentalFareCalculator";
...
export default class RentalfareScreen extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  componentDidMount() {
    ...
    var pkg = this.props.navigation.getParam("package");
    //alert (pkg);
    this.setState({
      pakg: pkg,
    });
    ...
    Data.once("value", (rates) => {
      if (rates.val()) {
        ...
        for (var i = 0; i < carTypeWiseRate.car_type.length; i++) {
          if (
            carTypeWiseRate.car_type[i].name == carType &&
            carTypeWiseRate.car_type[i].category == "Bike"
          ) {
              ...
              () => {
                ...
                this.getDirections(
                  '"' +
                    ...
                    ", " +
                    this.state.region.wherelongitude +
                    '"',
                  '"' +
                    this.state.region.droplatitude +
                    ", " +
                    this.state.region.droplongitude +
                    '"'
                );
                //fetch UserDetails
  ...
  async getDirections(startLoc, destLoc) {
    try {
      var resp = await fetch(
        `https://maps.googleapis.com/maps/api/directions/json?origin=${startLoc}&destination=${destLoc}&key=${google_map_key}`
      );
      var respJson = await resp.json();

      var fareCalculation = rentalfarehelper(
                              ...
                              if (
                                ...
                                if (allUsers[key].location) {
                                  ...
                                  if (originalDistance <= 10) {
                                    // Request will be send if distance less than 10 km
                                    if (
                                      allUsers[key].carType ==
                                      this.state.carType
                                    ) {
                            ...
                            if (
                              ...
                              if (allUsers[key].location) {
                                ...
                                if (originalDistance < 10) {
                                  // Request will be send if distance less than 10 km
                                  if (
                                    allUsers[key].carType == this.state.carType
                                  ) {
                                    allUsers[key].driverUid = key;
                                    arr.push(allUsers[key].driverUid);
                                    ...
                                    firebase
                                      ...
                                      .set(data); //send request to driver who are available
                                    this.sendPushNotification(
                                      allUsers[key].driverUid,
                                      bookingKey,
                                      languageJSON.new_booking_request_push_notification
              ...
              <View style={styles.upperContainer}>
                <View style={styles.fareDetailsContainer}>
                  <View style={styles.fareDetails}>
                    <Text style={styles.fareTitleText}>Est KM</Text>
                    <Text style={styles.fareTitleText}>Est Fare</Text>
                    <Text style={styles.fareTitleText}>Package Details</Text>
                    <Text
                      style={styles.fareTitleText}
                      style={{ display: "none" }}
                    >
                      {languageJSON.convenience_free}
                    </Text>
                  </View>
                  ...
                  <View style={styles.farePriceContainer}>
                    ...
                    <Text style={styles.farePriceText}>
                      {Currency} {this.state.fareCost}{" "}
                    </Text>
                    <Text style={styles.farePriceText}>
                      {this.state.pakg}hrs/{this.state.pakg * 10}KM{" "}
                    </Text>
                    <Text
                      style={styles.farePriceText}
                      style={{ display: "none" }}
                    >
                      {Currency}{" "}
                ...
                <View style={styles.totalPriceContainer}>
                  <View style={styles.totalPrice}>
                    <Text style={styles.totalPriceText}>
                      {languageJSON.total_fare}
                    </Text>
                    <Text style={styles.taxText}>
                      {languageJSON.inclusive_tax}{" "}
                    </Text>
                  </View>
  ...
  render() {
    console.log("estimateFare: ", this.state.estimateFare);
    console.log("estimateFare: ", this.state.distance);
    console.log("estimateFare: ", this.state.fareCost);
          ...
          <View style={styles.priceDetailsContainer}>
            <View style={styles.priceDetailsLeft}>
              ...
              <View style={styles.iconContainer}>
                {this.state.estimateFare !== null ? (
                  <Text style={styles.priceText}>
                    {" "}
                    {Currency} {this.state.estimateFare}
                  </Text>
                ) : (
...
const styles = StyleSheet.create({
  ...
  bottomContainer: { flex: 3.5, alignItems: "center" },
  offerContainer: {
    flex: 1,
