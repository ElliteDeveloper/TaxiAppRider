...
export default class OutstationScreen extends React.Component {
    ...
    //   allCabs = '';
     ...
     allCarsData(){
        const cars=firebase.database().ref('rates/car_type').orderByChild('category').equalTo('Keke');
      ...
      getDriverTime(startLoc,destLoc) {
        return new Promise(function(resolve, reject) {
            fetch(`https://maps.googleapis.com/maps/api/distancematrix/json?units=imperial&origins=${startLoc}&destinations=${destLoc}&key=${ google_map_key }`)
                ...
                .then((res) => 
                    resolve({
                        distance_in_meter:  100, //res.rows[0].elements[0].distance.value,
                        time_in_secs:  30,//res.rows[0].elements[0].duration.value,
                        timein_text:  '30minutes',//res.rows[0].elements[0].duration.text
                    })
                )
                .catch(error=>{
                    reject(error);
        ...
        return (
          <View style={styles.mainViewStyle}>
            <Header 
                ...
                leftComponent={{icon:'md-arrow-back', type:'ionicon', color: colors.WHITE, size: 30, component: TouchableWithoutFeedback,onPress: ()=>{this.props.navigation.navigate('Map');} }}
                centerComponent={<Text style={styles.headerTitleStyle}>Keke Rides</Text>}
            ...
            <View style={styles.myViewStyle}>
                ...
                <View style={styles.iconsViewStyle}>
                    <TouchableOpacity onPress={()=>{this.props.navigation.navigate('Searchdaily',{from:"where",whereText:this.state.whereText,dropText:this.state.dropText,old:this.state.passData});}} style={styles.contentStyle}>
                    ...
                    <TouchableOpacity onPress={()=>{this.props.navigation.navigate('Searchdaily',{from:"drop",whereText:this.state.whereText,dropText:this.state.dropText,old:this.state.passData});}} style={styles.searchClickStyle}>
                        <View style={styles.textIconStyle}>
            ...
            <View style={styles.compViewStyle}>
                ...
                <ScrollView horizontal={true} style={styles.adjustViewStyle} showsHorizontalScrollIndicator={false}>
                    ...
                    <TouchableOpacity key={key} style={styles.cabDivStyle} onPress={()=>{this.selectCarType(prop,key)}} disabled={prop.minTime == ''} >
                        ...
                        backgroundColor: prop.active == true?'#ffffff':'#ffffff', borderColor: prop.active == true?'#000':'#697174'}]
                        }>
                ...
                <View style={{flex:0.5}}>
                    <Button
                        ...
                        buttonStyle={{width: width, height:70, backgroundColor: colors.GREY.btnPrimary,elevation: 0}}
